...
- **`data-viz-storytelling-healy`** — Chart selection rubric, honesty checklist (Healy Ch. 1), narrative structure (claim-evidence-caveat), and model-diagnostic plot catalog. Use *before* this skill when deciding *which* chart to draw or reviewing figures for misleading patterns.
- **`infographics`** — Template-driven visual design for non-technical audiences (Nano Banana Pro). Use when the primary goal is visual impact rather than statistical completeness.
- **`seaborn`** — Quick statistical plots with attractive defaults. Good starting point before applying publication refinements from this skill.
